...
+ (myProtractorLayer *)drawProtractorLayer
{
    ...
    layer.position = CGPointMake(187.5, kScreenHight - layer.protractor_nop);
...
- (void)redrawIncludedAngleLineFromAngle:(CGFloat)from toAngle:(CGFloat)to
{
    ...
    // 角度值更新
    self.angle_layer.string = [NSString stringWithFormat:@" %.2f°", radia_to_angle(self.endAngle - M_PI_2)];
    if (self.callback)
    {
